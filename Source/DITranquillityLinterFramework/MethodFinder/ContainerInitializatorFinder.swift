...
final class ContainerInitializatorFinder {
	...
	static func findContainerStructure(dictionary: [String : SwiftType], project: XcodeProj) -> ContainerPart? {
		...
		for structureInfo in possibleContainerValues {
			if let mainContainerPart = recursivelyFindContainerInitialization(list: structureInfo.substructure, file: structureInfo.file, contents: structureInfo.file.contents.bridge(), dictionary: dictionary, project: project) {
	...
	private static func recursivelyFindContainerInitialization(list: [SourceKitStructure], file: File, contents: NSString, dictionary: [String : SwiftType], project: XcodeProj) -> ContainerPart? {
		...
		if let containerInitIndex = list.index(where: isContainerInitialization), containerInitIndex > 0 {
			...
			return ContainerPart(substructureList: list, file: file, collectedInfo: dictionary, currentPartName: nil, project: project, files: [])
		}
		
		for substructureInfo in list {
			if let mainContainerPart = recursivelyFindContainerInitialization(list: substructureInfo.substructures ?? [], file: file, contents: contents, dictionary: dictionary, project: project) {
				return mainContainerPart
			}
		}
		return nil
